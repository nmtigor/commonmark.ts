...
export type Equals<A1 extends any, A2 extends any> =
  (<A>() => A extends A2 ? true : false) extends
    <A>() => A extends A1 ? true : false ? true : false;
...
export type NoRequiredParams<T extends ParameterizedObject> = T extends any
  ? undefined extends T["params"] ? T["type"] : never
  : never;
...
> = T extends any ? ConditionalRequired<{
    type: T["type"];
    params?:
      | T["params"]
      | (({ context, event }: {
...
type PartialEventDescriptor<TEventType extends string> = TEventType extends
  `${infer TLeading}.${infer TTail}`
...
export type InvokeConfig<
  ...
  TEmitted extends EventObject,
  TMeta extends MetaObject,
> = IsLiteralString<TActor["src"]> extends true ? DistributeActors<
    TContext,
    TEvent,
    TActor,
    TAction,
...
export interface MachineTypes<
  ...
  TMeta extends MetaObject,
> extends
  SetupTypes<TContext, TEvent, never, TTag, TInput, TOutput, TEmitted, TMeta> {
  actors?: TActor;
  actions?: TAction;
  guards?: TGuard;
  delays?: TDelay;
...
export interface SendToActionOptions<
  ...
  TDelay extends string,
> extends
  RaiseActionOptions<TContext, TExpressionEvent, TParams, TEvent, TDelay> {
}
...
export interface SendToActionParams<
  ...
  TDelay extends string,
> extends
  SendToActionOptions<TContext, TExpressionEvent, TParams, TEvent, TDelay> {
  event:
    | TSentEvent
...
type EventDescriptorMatches<TEventType extends string, TNormalizedDescriptor> =
...
> = string extends TEvent["type"] ? TEvent
  : NormalizeDescriptor<TDescriptor> extends infer TNormalizedDescriptor
    ? TEvent extends any
      ? true extends
        EventDescriptorMatches<TEvent["type"], TNormalizedDescriptor> ? TEvent
      : never
    : never
  : never;
...
export type SnapshotFrom<T> = ReturnTypeOrValue<T> extends infer R
  ? R extends ActorRef<infer TSnapshot, infer _, infer __> ? TSnapshot
  ...
  : R extends
    ActorScope<infer TSnapshot, infer _TEvent, infer _TEmitted, infer _TSystem>
    ? TSnapshot
  : never
  : never;
export type EventFromLogic<TLogic extends AnyActorLogic> = TLogic extends
  ActorLogic<
    infer _TSnapshot,
    infer TEvent,
    infer _TInput,
...
type ResolveEventType<T> = ReturnTypeOrValue<T> extends infer R
  ...
  : R extends MachineSnapshot<
    ...
    infer _TStateSchema
  > ? TEvent
  : R extends ActorRef<infer _TSnapshot, infer TEvent, infer _TEmitted> ? TEvent
  : never
  : never;
...
export type ContextFrom<T> = ReturnTypeOrValue<T> extends infer R
  ...
  : R extends MachineSnapshot<
    ...
    infer _TStateSchema
  > ? TContext
  : R extends Actor<infer TActorLogic> ? TActorLogic extends StateMachine<
      infer TContext,
      infer _TEvent,
      infer _TChildren,
      infer _TActor,
...
export type ToChildren<TActor extends ProvidedActor> = string extends
  TActor["src"] ? Record<string, AnyActorRef>
  : Compute<
    ...
    & {
      ...
      undefined extends TActor["id"] ? "include"
        : string extends TActor["id"] ? "include"
        : "exclude"
    ]
  >;
...
> =
  | (TSchema extends {
    ...
    : `${TParentKey}.${TKey}`)
  | (TSchema["states"] extends Record<string, any> ? Values<
      {
